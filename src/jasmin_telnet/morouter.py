...
class MORouter(BasicManager):
    "MORouter for managing MT Routes"

    def __init__(self, conn: TelnetConnection):
        super().__init__(conn=conn, module="morouter", lookup_field="order")
    ...
    def _parseSingleStatus(self, status_list: list) -> dict[str, str | int | float | bool]:
        ...
        return {
            ...
            "Type": status_list[1],
            "Rate": status_list[2],
            "Connector(s)": re.findall("http\(.*?\)", " ".join(status_list)) + re.findall("smpps\(.*?\)", " ".join(status_list)),
    ...
    def _getRequiredAttributes(self, data: dict[str, str | int | float | bool]) -> list[str]:
        ...
        elif data["type"].lower() == "staticmoroute":
            requiredAttributes.extend(["filters", "connector", "order"])

        elif data["type"].lower() == "defaultroute":
            requiredAttributes.extend(["connector"])

        elif data["type"].lower() == "randomroundrobinmoroute":
            requiredAttributes.extend(["filters", "connectors", "order"])

        elif data["type"].lower() == "failovermoroute":
            requiredAttributes.extend(["filters", "connectors", "order"])
